...
def setup_handling(host):
    asyncio.get_event_loop().add_signal_handler(signal.SIGINT, lambda: asyncio.create_task(handle(host)))

'''
Starts up a client user
'''
async def client_main(args):
    return Client(Host(args.server_ip, args.server_port), args.id)
...
if __name__ == "__main__":
    ...
    ap = ArgumentParser()
    ap.add_argument("--pdb", type=str, default="pdb.json", help = "the password database that should be referenced")
